...
<div>
	<p>この度、LIXIL Family STORE  LIXIL会員登録申請いただき、ありがとうございます。</p>
	<p>まだ会員登録は完了しておりません。</p>
	<p>このメールに記載された、『本登録用のURL』から本登録をお願い致します。</p>
</div>
<div>
    <p>つきましては、 <b>{{ $date_limit}} </b> までに、</p>
	<p>本登録を下記手順にそって行っていただきますよう、お願い申し上げます。</p>
	<p style="font-size: 12px; font-style: normal;"> <i>※尚、上記の期限を過ぎた場合は、再度、最初の会員登録申請から行うことになりますので、ご注意ください。</i></p>
</div>
...
<div>
	<p>＜本登録用のURL＞</p>
    LIXIL Family STORE 会員本登録： <a href="{{ route('member.form.register.password', ['guid' => $guid]) }}" target="_blank">
        {{ route('member.form.register.password', ['guid' => $guid]) }}
    </a>
</div>
<div>
